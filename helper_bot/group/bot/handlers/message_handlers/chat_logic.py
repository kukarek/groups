...
class ChatLogic:
    ...
    RULES_LINK = "Чтобы узнать правила размещения, свяжитесь с администратором группы"
    PAYMENT = "Для получения реквизитов свяжитесь с администратором группы"
    EXAMPLE_WORDS = "без опыта, официант, бармен, подработка, стройка, шабашка, оплата сразу"
    ...
    def none_status_handler(self, user_id, message_text, group_id):
    ...
    def start_status_handler(self, user_id, message_text, group_id):
    ...
    def employer_status_handler(self, user_id, message_text, group_id):
        ...
        if message_text == "/start" or message_text == "Главное меню": #откат до статуса старт
            ...
            return Response(message=self.RULES_LINK)
        
        elif message_text == "Реквизиты":
            
            return Response(message=self.PAYMENT)
    ...
    def employer_and_admin_status_handler(self, user_id, message_text, group_id):
    ...
    def editing_status_handler(self, user_id, message_text, group_id):
        ...
        if message_text == "/start" or message_text == "Главное меню":
            ...
            return Response(message=self.EXAMPLE_WORDS, 
                            keyboard=create_applicant_keyboard(keywords=sql.get_keywords(user_id=user_id, group_id=group_id)))
    ...
    def applicant_status_handler(self, user_id, message_text, group_id):
